...
export default class BookmarkPage {
  ...
  async render() {
    return `
      ...
      <section class="container">
        <h2 class="section-title">Daftar Laporan Berita Terimpan</h2>
  ...
  async afterRender() {
    // alert('Halaman berita tersimpan akan segera hadir!');
  ...
  async populateBookmarkedList(message, stories) {
    if (stories.length <= 0) {
  ...
  populateBookmarkedError(message) {
